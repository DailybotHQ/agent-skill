...
- feat(report): three-trigger model — deliverables (research, docs, plans), broad edit, hook reminder bias
- docs(report): expand significance, examples, writing-guide, hooks default-bias section
- docs(shared): document `report.mode` and `report.soft_turn_threshold` profile keys (requires matching CLI release)
